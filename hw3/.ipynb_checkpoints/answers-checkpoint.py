...
part3_q1 = r"""
The aliding windows attention at at on layes will
have contecst of size 2 * window_size + 1. whwn we stack encoder layes
...
wich itself containes contecst of size 2 * window_size + 1. therfore,
the total contecst size will  O((2 * window_size + 1) ** num_layers). 
"""

part3_q2 = r"""
I will compress the keys and values using a lernable layer.
the layer will be a  MLP that from each window of size k  will
generate single token representation. we can have strid s.
we will choose k and s such that the new sequense will be of length w.
than, i will recive K' dims are [Batch, *, w, Dims]
v' dims are [Batch, *, w, Dims].
...
all the infurt, but compressed, awer atention will still contain global information.
"""
